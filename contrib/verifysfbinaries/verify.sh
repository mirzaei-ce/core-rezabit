...
###   This script attempts to download the signature file SHA256SUMS.asc from rezabit.org
...
WORKINGDIR="/tmp/rezabit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://rezabit.org/bin/"
VERSIONPREFIX="rezabit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'rezabit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Rezabit Core binary release signing key installed?"
   fi
