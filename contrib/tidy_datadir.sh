...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Rezabit database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Rezabit datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Rezabit datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Rezabit 0.7 datadir."
    ;;
  3)
    echo "Detected Rezabit pre-0.8 datadir."
    ;;
  4)
    echo "Detected Rezabit 0.8 datadir."
    ;;
esac
